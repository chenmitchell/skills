import type { OpenClawPlugin, PluginAPI } from "./types.js";
...
import type { ScanLocation } from "./types.js";
...
const plugin: OpenClawPlugin = {
  ...
  register(api: PluginAPI) {
    ...
    log.info(
      `Initializing AI Sentinel v0.1.1 [mode=${config.mode}, threshold=${config.threatThreshold}]`,
    );
    ...
    if (config.agentOverrides.length > 0) {
      log.info(
        `Agent overrides: ${config.agentOverrides.map((o) => o.agentId).join(", ")}`,
      );
    }

    // Initialize API reporter if configured
    let reporter: APIReporter | null = null;
    if (
      config.apiKey &&
      config.apiUrl &&
      ...
      log.info(
        `API reporting enabled [mode=${config.reportMode}] → ${config.apiUrl}`,
      );
    ...
    api.on(
      "message_received",
      (event: any, ctx: any) => messageReceivedHook({
    ...
    api.on(
      "before_tool_call",
      (event: any, ctx: any) => beforeToolCallHook({
    ...
    api.on(
      "tool_result_persist",
      (event: any, ctx: any) => toolResultPersistHook({
        toolName: event.toolName,
    ...
    api.on(
      "before_agent_start",
      (event: any, ctx: any) => beforeAgentStartHook({
        sessionKey: ctx.sessionKey,
        agentId: ctx.agentId,
      }),
      { priority: 100 },
    ...
    api.registerTool({
      ...
      description:
        "Manually scan text for prompt injection threats. Use when you suspect content may be adversarial.",
      parameters: {
        text: {
          type: "string",
        ...
        location: {
          type: "string",
          description:
            'Where the text came from: "message", "tool_result", "tool_params", or "system"',
          required: false,
        },
      },
      execute(params: Record<string, unknown>) {
        const text = params.text as string;
        const location = (params.location as ScanLocation) ?? "message";
